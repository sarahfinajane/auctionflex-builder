window.addEventListener("DOMContentLoaded", () => {
  ...
  // Make the title input itself enforce 49 characters.
  if (fields.title) {
    fields.title.maxLength = MAX_TITLE_LENGTH;
  }
  ...
  function buildDescription(lot) {
    return document.getElementById("masterDescription").value
      .replaceAll("{TITLE}", safeTitle(lot.title || ""))
  ...
  function loadCurrentLot() {
    ...
    fields.title.value = safeTitle(lot.title);
  ...
  function looksLikeBadTitle(line) {
    ...
    const badWords = [
      ...
      "add to cart", "buy now", "department", "aisle", "model #",
      "sku", "upc", "item #", "internet #", "store sku", "showing",
      "sort by", "filter", "retail", "condition"
    ];
  ...
  function titleScore(line) {
    ...
    const productWords = [
      ...
      "vanity", "toilet", "tile", "mirror", "mower", "battery", "charger"
    ];
    ...
    if (line.length >= 18 && line.length <= MAX_TITLE_LENGTH) score += 14;
    if (line.length > MAX_TITLE_LENGTH && line.length <= 110) score += 4;
  ...
  async function runOCR() {
    ...
    try {
      ...
      if (title) fields.title.value = safeTitle(title);
  ...
  function getExportValidationErrors() {
    const errors = [];
    ...
    lots.forEach((lot, index) => {
      ...
      if (!lot.lotNumber) {
        errors.push(`Line ${index + 1}: Lot # is missing.`);
      }

      if (!safeTitle(lot.title)) {
        errors.push(`Line ${index + 1}: Title is missing.`);
      }

      if (safeTitle(lot.title).length > MAX_TITLE_LENGTH) {
        errors.push(`Line ${index + 1}: Title is over ${MAX_TITLE_LENGTH} characters.`);
      }
    });
  ...
  function downloadCSV() {
    ...
    const errors = getExportValidationErrors();
    ...
    const rows = lots
      .filter(lot => lot.lotNumber && safeTitle(lot.title))
      .map(lot => [
        lot.lotNumber,
        lot.sellerCode,
        safeTitle(lot.title),
  ...
  async function downloadImageZip() {
    ...
    const missingLotPhotos = lots.filter(lot =>
      !lot.lotNumber && (lot.stock.length || lot.person.length)
    );

    if (missingLotPhotos.length) {
      alert("Some photos are attached to lots with no Lot #. Add Lot # before downloading images.");
      return;
    }
